...
To do so, double click on ``devtools\run-conda-lock.bat`` or execute it from command line::
    ...
    $ [path\to\geoapps-utils]\devtools\run_conda_lock.bat
...
For development, you need a **conda** environments. you can install it running the ``setup-dev.bat`` or::
    ...
    $ [path\to\geoapps-utils]\devtools\setup-dev.bat
...
.. code-block:: bash
    ...
    pytest --cov=geoapps_utils --cov-report html tests
...
**geoapps-utils** and its dependencies. Make sure to turn off the ``Use Conda Package Manager``
...
select ``pytest in tests``, and add the following option in the ``Additional Arguments`` field::
    ...
    --cov=geoapps_utils --cov-report html
